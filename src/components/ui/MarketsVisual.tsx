...
const legends = [
  ...
  { label: 'Venue B', color: 'rgba(255,255,255,0.5)' },
  { label: 'Venue C', color: 'rgba(250,204,21,0.6)' },
  { label: 'Venue D', color: 'rgba(52,211,153,0.5)' },
];
      ...
      >
        <defs>
          <pattern id="markets-grid" width="20" height="20" patternUnits="userSpaceOnUse">
            <circle cx="10" cy="10" r="0.5" fill="white" opacity="0.08" />
          </pattern>
        </defs>
      ...
      <div className="absolute top-2 right-2 flex flex-col gap-1">
        {legends.map((item) => (
          <div key={item.label} className="flex items-center gap-1.5">
            ...
            />
            <span className="text-[7px] font-mono text-white/25">{item.label}</span>
          </div>
        ))}
      </div>

      <div className="absolute top-1.5 left-2">
        <span className="text-[6px] font-mono text-white/15 tracking-widest uppercase">
          Overlay
        </span>
      </div>
    </div>
